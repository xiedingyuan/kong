...
local function add_extra_attributes(fields, opts)
  ...
  if opts._tags then
    table.insert(fields, {
      _tags = { type = "set", elements = { type = "string" } },
    })
  end
end
...
local function build_fields(entities)
  local fields = {
    ...
    { _apply_tags = { type = "array", elements = { type = "string" } } },
  }
  ...
  for _, f in ipairs(fields) do
    ...
    if fname == "plugins" then
      for plugin in pairs(plugin_set) do
        local _, err, err_t = plugin_loader.load_subschema(fdata.elements, plugin)

        if err_t then
          return nil, "schema for plugin '" .. plugin .. "' is invalid: " ..
                      tostring(Errors.new():schema_violation(err_t))
        end
        if err then
          return nil, err
        end
      ...
      if foreign_refs then
        for k, v in pairs(item) do
          ...
          if ref and v ~= null then
            ...
            table.insert(expected[entity][ref], { key = k, value = v, at = key or item_id or i })
          end
        end
      end
    ...
    for b, bs in pairs(as) do
      for _, k in ipairs(bs) do
        ...
        if not found then
          ...
          local msg = "invalid reference '" .. k.key .. ": " .. k.value .. "' (no such entry in '" .. b .. "')"
...
local function flatten(self, input)
  ...
  for entity, entries in pairs(by_id) do
    ...
    for id, entry in pairs(entries) do
      ...
      -- keep _tags, drop _comment and _ignore
      if entry._tags then
        flat_entry._tags = entry._tags
      end
...
function DeclarativeConfig.load(plugin_set)
  ...
  if not all_schemas then
    ...
    for _, entity in ipairs(core_entities) do
      local definition = utils.deep_copy(require("kong.db.schema.entities." .. entity), false)
      all_schemas[entity] = Entity.new(definition)
    end
  end

  for plugin in pairs(plugin_set) do
    local entities, err, err_t = plugin_loader.load_entities(plugin, plugin_loader.load_entity_schema)
