...
function plugin_loader.load_subschema(parent_schema, plugin)
...
function plugin_loader.load_entities(plugin, loader_fn)
  ...
  for name, schema_def in iterator(daos_schemas) do
    if name ~= "tables" and schema_def.name then
      local ret, err, err_t = loader_fn(plugin, schema_def)
