...
local function parse_declarative_config(config)
...
local function load_declarative_config(config, entities)
  if not config.database == "off" then
    return true
  end
...
function Kong.init()
  ...
  if kong.configuration.database == "off" then
    local err
    declarative_entities, err = parse_declarative_config(config)
